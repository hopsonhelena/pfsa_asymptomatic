...
fig2cd <- rbind(
  data.frame(
    Site = "Pfsa1",
    Sequencing_sample_ID = metadata$Seq.Sample.ID[pfsa1_mixed],
    HbS_genotype = metadata$HbS_gt[pfsa1_mixed],
    VAF = metadata$pfsa1_vaf[pfsa1_mixed]
  ),
  data.frame(
    Site = "Pfsa3",
    Sequencing_sample_ID = metadata$Seq.Sample.ID[pfsa3_mixed],
...
write.table(
  fig2ab, "results/figures/data/Figure2AB_genotype_counts.tsv",
  sep = "\t", row.names = FALSE, quote = FALSE
)
write.table(
  fig2cd, "results/figures/data/Figure2CD_VAF.tsv",
  sep = "\t", row.names = FALSE, quote = FALSE
)
