# Pipeline configuration. Sourced by all scripts: 
# source ./config.sh 
# Edit the settings under "Edit these". Everything below it is derived
# or fixed and should not need changing.

export PROJECT_DIR="$PWD"       
[[ -d "${PROJECT_DIR}/pipeline" ]] || {
    echo "config.sh: run from amplicon_pipeline/ (PROJECT_DIR=$PROJECT_DIR)" >&2
}                             
...
# See README for sources and preparation.
...
# Folder containing the sequencing run folders. Edit if your FASTQs are outside the repository
export DATA_DIR="${PROJECT_DIR}/data/runs"
...
# Nextera adapters, trimmed by 01_trim.sh with cutadapt
ADAPTER_R1="CTGTCTCTTATACACATCT"
ADAPTER_R2="CTGTCTCTTATACACATCT"
MIN_LENGTH=20

# Slurm array range: 0 to N_RUNS-1
N_RUNS=${#ALL_RUNS[@]}  