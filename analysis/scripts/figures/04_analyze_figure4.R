...
paired_vaf <- mixed_either[, c(
  "seq_sample_id", "screening_id", "HbS_gt", "pfsa1_vaf", "pfsa3_vaf"
)]
names(paired_vaf) <- c(
...
write.table(
  observed_expected, "results/tables/Figure4A_observed_expected.tsv",
  sep = "\t", row.names = FALSE, quote = FALSE
)
write.table(
  ld_statistics, "results/tables/Figure4A_LD_r2.tsv",
  sep = "\t", row.names = FALSE, quote = FALSE
)
write.table(
  paired_vaf, "results/tables/Figure4B_paired_VAF.tsv",
  sep = "\t", row.names = FALSE, quote = FALSE
)
